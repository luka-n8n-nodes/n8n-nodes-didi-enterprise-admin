...
const AddOperate: ResourceOperations = {
	...
	value: 'add',
	action: '新增员工',
	description: 'POST /river/Member/single',
	order: 20,
	options: [
		{
			displayName:
				'接口要求连续新增间隔 ≥150ms，节点已自动限速。证件号/出生日期会按官方规则做 AES128 加密。文档：<a href="https://opendocs.xiaojukeji.com/version2024/11155" target="_blank">员工新增</a>',
			name: 'notice',
			type: 'notice',
			default: '',
		},
		...
		},
		{
			...
			type: 'string',
			default: '',
			description: '主键为手机号时必填。海外/港澳台格式：+86 12345678910',
		},
		{
			displayName: '工号',
	...
	async call(this, index) {
		...
		const data = encryptMemberPrivacy(
			pickFilled({
				member_type: this.getNodeParameter('member_type', index, 0),
				...
				use_company_money: this.getNodeParameter('use_company_money', index, 0),
		...
		const body = pickFilled({
			send_message: this.getNodeParameter('send_message', index, 0),
			has_card_info,
